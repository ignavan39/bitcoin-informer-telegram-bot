...
const sendPost = async (postInfo, coinInfo, channelId) => {
    ...
    let postMessage = `${postInfo.name[0].toUpperCase() + postInfo.name.slice(1)} Price $${coinInfo.last} 
...
$${coinInfo.last}/$${coinInfo.high}`
    ...
    return bot.telegram.sendMessage(channelId, postMessage).then(ctx => ctx.message_id)
    
}

const editPost = async (postInfo, coinInfo, channelId, prevPostId) => {
    let isDeleted = bot.telegram.deleteMessage(channelId, prevPostId).then(ctx => ctx)
    if(isDeleted){
