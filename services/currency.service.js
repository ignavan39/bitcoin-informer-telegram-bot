...
const { checkAllCoinsService, checkAllVsCurrencySercice } = require("./check.service");
...
const fetchAllVsCurrencies = async () => {
    const rawRes = await fetch("https://api.coingecko.com/api/v3/simple/supported_vs_currencies")
    return await rawRes.json()
}
...
const currencyService = async (currency_name, vsCurrency) => {
    ...
    const rawRes = await fetch(`https://api.coingecko.com/api/v3/coins/${currency_id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false`)
